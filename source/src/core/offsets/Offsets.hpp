...
namespace offsets
{
	// Set by ResolveViewMatrix at startup if pattern scan succeeds
	inline uint64_t resolvedViewMatrixVA = 0;
	// client.dll — cs2-dumper 2026-08-27
	inline DWORD viewMatrix            = 0x23CB830;
	inline DWORD entityList            = 0x2571230;
	inline DWORD localPlayerController = 0x23A0F30;
	inline DWORD globalVars            = 0x20AF5F0;
	inline DWORD plantedC4             = 0x2390A18;
	inline DWORD localPlayerPawn       = 0x23C6268;
	inline DWORD csgoInput             = 0x23DBC80;
	inline DWORD viewAngles            = 0x23DC308;

	// engine2.dll
	inline DWORD buildNumber           = 0x60F594;

	namespace controller {
		constexpr std::ptrdiff_t m_iPing = 0x830; // uint32
		constexpr std::ptrdiff_t m_hPawn = 0x6BC; // CHandle<C_BasePlayerPawn> — base class only
		constexpr std::ptrdiff_t m_hPlayerPawn = 0x914; // CHandle<C_CSPlayerPawn> — actual game pawn
	...
	namespace pawn {
		...
		constexpr std::ptrdiff_t m_lifeState = 0x354; // uint8 — 0 = LIFE_ALIVE
		constexpr std::ptrdiff_t m_iTeamNum = 0x3E7; // uint8
		constexpr std::ptrdiff_t entityIdentityStride = 0x70; // CEntityIdentity
		...
		constexpr std::ptrdiff_t m_angEyeAngles = 0x3350; // QAngle [pitch, yaw, roll]
	}

	namespace bomb {
		constexpr std::ptrdiff_t m_isPlanted = 0x8; // unk
		constexpr std::ptrdiff_t m_bC4Activated = 0x11E8; // bool - C_PlantedC4
		constexpr std::ptrdiff_t m_nBombSite = 0x11A4; // int32 - C_PlantedC4
