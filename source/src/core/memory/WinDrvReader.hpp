...
// RTCore64.sys (Micro-Star INT'L CO., LTD., MSI Afterburner monitoring
// driver, WHCP-signed). PDB path in the on-disk binary confirms the WDK
// 7600.16385.1 build; export table references MmMapIoSpace, MmUnmapIoSpace,
// ZwMapViewOfSection, ZwOpenSection, HalTranslateBusAddress, plus the
// HalGetBusDataByOffset / HalSetBusDataByOffset PCI config primitives.
// Signer chain: MICRO-STAR INTERNATIONAL CO., LTD. → GlobalSign GCC R45
// EV CodeSigning CA 2020 → attribute-signed via Microsoft Windows Third
// Party Component CA 2014 → Microsoft Windows Hardware Compatibility
// Publisher. Loads on stock Windows 11 with Secure Boot (and, on this
// target, with SecurityServicesRunning=0 / SmartAppControl=Off / no
// vulnerable-driver blocklist enforcement — verified before the swap).
//
// Device path: hardcoded at DriverEntry (visible in the driver's .rdata
// as a bare UNICODE_STRING). Publishes:
//   \Device\RTCore64
//   \DosDevices\RTCore64
// User-mode path: "\\.\RTCore64". The SERVICE name we register under
// (main.cpp GetSvcName — machine-stable hex) affects only SCM registration
// and the on-disk .sys filename, NOT the device path. The device-path
// IOC is mitigated by idle-mode handle closure between scans; the handle
// is open only during the tight IOCTL burst.
//
// IRP_MJ_CREATE gate:
//   Trivial no-op that returns STATUS_SUCCESS. DACL string embedded in
//   the driver is D:P(A;;GA;;;SY)(A;;GA;;;BA) — SYSTEM + Built-in Admins
//   full access. Elevated admin token passes; standard user is refused
//   by the DACL before CREATE runs. No arm sequence, no per-handle
//   activation cookie, no process-identity check — the read handler
//   dispatches on the first IOCTL after CreateFile.
//
// IOCTL surface used here:
//   0x80002048  ← READ/WRITE phys memory (MmMapIoSpace + rep movs, 64-bit PA)
//
// This is the same primitive CVE-2019-16098 disclosed. RTCore64 has other
// IOCTLs (MSR read/write, PCI config space, I/O port access), but the
// ESP-side code only ever needs physical-memory reads, so 0x80002048 is
// the sole code the reader dispatches to.
//
// IOCTL 0x80002048 primitive (METHOD_BUFFERED, input+output share the
// same system buffer):
//   InputBufferLength  == 0x30
//   OutputBufferLength >= 0x30            (data lives in the same buffer)
//   Request layout (bytes 0x00..0x2C are inputs; 0x18..end is the read
//   output slot):
//     +0x00  uint8_t  reserved[8]        (zero)
//     +0x08  uint64_t phys_addr          ← full 64-bit PA
//     +0x10  uint32_t bytes_read         (driver writes back the byte count)
//     +0x14  uint32_t granularity        ← 1, 2, 4, or 8 — matches rep movs stride
//     +0x18  uint8_t  data[length]       ← in-place read output
//     +0x28  uint32_t length_bytes       ← total bytes to read
//     +0x2C  uint32_t reserved           (zero)
//   Driver: MmMapIoSpace(PA, length, MmNonCached) → rep movsb/w/d/q
//     from mapped region → output slot → MmUnmapIoSpace → IoStatus.Status
//     = STATUS_SUCCESS.
//   User side: DeviceIoControl returns TRUE, lpBytesReturned = 0x30 (or
//   whatever OutputBufferLength was passed); the reader copies bytes from
//   the shared buffer's +0x18 slot into the caller's target.
//
// Trade-offs vs. the NTIOLib build we shipped before:
//   + No arm sequence — first IOCTL after CreateFile succeeds
//   + No process-identity gate — elevated admin is enough, no SYSTEM
//     token duplication needed
//   + Same 64-bit PA support (MmMapIoSpace path, not a MmGetPhysicalAddress
//     32-bit truncation)
//   - Older PDB (WDK 7600) is on some older LOLDrivers datasets; blocklist
//     status verified per-host at load time (main.cpp path check + HVCI
//     compatibility gate is up-front now, not at the reader).
//   - Device path \Device\RTCore64 is a known IOC — same mitigation as
//     before: idle-mode reopen keeps the handle out of our process handle
//     table between scans; the .sys on disk is renamed to a machine-stable
//     hex; the SCM service name is likewise stable-hex.
#define IOCTL_RTC_READ_PHYS             0x80002048u
#define RTC_REQ_SIZE                    0x30u
#define RTC_REQ_DATA_OFFSET             0x18u
#define RTC_REQ_MAX_PAYLOAD             (RTC_REQ_SIZE - RTC_REQ_DATA_OFFSET)    // 24 bytes per IOCTL

// RTCore64 IOCTL 0x80002048 return-contract summary:
//
//   Path                                     IoStatus.Status       Information
//   Success (any granularity*count read)     STATUS_SUCCESS  (0)   = 0x30 (buf size)
//   MmMapIoSpace fail (bad PA / refused)     STATUS_UNSUCCESSFUL   0
//                                              (0xC0000001)
//   InputBufferLength != 0x30                STATUS_INVALID_       0
//                                              PARAMETER (0xC000000D)
//   OutputBufferLength < 0x30                STATUS_INVALID_       0
...
public:
    ...
    // RTCore64 hardcodes its \Device\RTCore64 UNICODE_STRING at DriverEntry.
    // The user-mode path is fixed and cannot be renamed without patching the
    // signed driver and breaking the WHCP signature. What we CAN control:
    ...
    //     table clean of \Device\RTCore64 between IOCTL bursts)
    ...
    bool Open() {
        ...
        DBG_PRINT("[rtc] Opening device...\n");

        // Single-shot open. RTCore64's DACL is SYSTEM+BA GA and the CREATE
        // handler is a no-op that accepts any elevated admin handle — no
        // access-mode escalation loop, no arm sequence, no handle-context
        // probe. The only retryable condition is ERROR_FILE_NOT_FOUND, the
        // brief window between SCM StartService returning and the symlink
        // \DosDevices\RTCore64 being visible via \\.\ resolution.
        const char* base       = GetDevicePath();
        const char* pathGlobal = "\\\\.\\GLOBALROOT\\Device\\RTCore64";
        DWORD lastErr = 0;
        for (int attempt = 0; attempt < 10 && m_hDevice == INVALID_HANDLE_VALUE; attempt++) {
            m_hDevice = CreateFileA(base,
            ...
            // Session-isolation fallback: some AC / policy configurations
            // strip \\.\ resolution for our session but leave the raw
            // NT-namespace path intact. Try it once before giving up.
            m_hDevice = CreateFileA(pathGlobal,
                                    GENERIC_READ | GENERIC_WRITE,
                                    FILE_SHARE_READ | FILE_SHARE_WRITE,
                                    nullptr, OPEN_EXISTING,
                                    FILE_ATTRIBUTE_NORMAL, nullptr);
        ...
        if (m_hDevice == INVALID_HANDLE_VALUE) {
            DBG_PRINT("[rtc] Failed to open device: err=%lu\n", lastErr);
            if (lastErr == 2)      DBG_PRINT("[rtc]   (device not found -- driver not loaded or bad service name)\n");
            else if (lastErr == 5) DBG_PRINT("[rtc]   (access denied -- token DACL rejects; not elevated?)\n");
            printf(skCrypt("[SysMonitor] RTCore64 device open FAILED err=%lu\n"),
                   (unsigned long)lastErr);
            return false;
        }
        DBG_PRINT("[rtc] Device opened: handle=0x%p\n", (void*)m_hDevice);
        printf(skCrypt("[SysMonitor] RTCore64 device open\n"));

        // Probe: read 4 bytes at PA=0x1000. This exercises the full driver
        // path (MmMapIoSpace, rep movs into shared buffer, MmUnmapIoSpace)
        // in the same shape the hot path uses. A stub or dead-case IOCTL
        // in this driver would return err=87 / err=24; a real handler
        // returns ok=1 with the mapped bytes copied.
        uint32_t probe = 0;
        bool ok = RtcReadOnce(0x1000, 4, 1, &probe, sizeof(probe));
        if (!ok) {
            printf(skCrypt("[SysMonitor] RTCore64 probe FAILED err=%lu -- device up but IOCTL 0x%08X rejected\n"),
                   (unsigned long)GetLastError(), (unsigned)IOCTL_RTC_READ_PHYS);
            CloseHandle(m_hDevice);
            m_hDevice = INVALID_HANDLE_VALUE;
            return false;
        }
        printf(skCrypt("[SysMonitor] RTCore64 probe OK  PA=0x1000 dword -> 0x%08X\n"),
               (unsigned)probe);
        return true;
    }
    ...
    // concurrent RtcReadOnce mid-flight either (a) captured hUse under the
    ...
    // Direct physical read — bypasses the page cache. Kept as a separate
...
private:
    ...
    void NoteSivFail(SivFail cls, uint64_t pa, DWORD size, DWORD returned, DWORD err) {
        ...
        if (n < kIoFailLogCap) {
            printf(skCrypt("[SysMonitor] RTCore64 IOCTL 0x%08X failed: %s  pa=0x%llX  bytes=%u  returned=%u  err=%lu\n"),
                   (unsigned)IOCTL_RTC_READ_PHYS,
                   SivFailName(cls),
            ...
            printf(skCrypt("[SysMonitor] RTCore64 IOCTL 0x%08X: further %s failures suppressed (cap=%d)\n"),
                   (unsigned)IOCTL_RTC_READ_PHYS, SivFailName(cls), kIoFailLogCap);
        }
    }

    // (NTIOLib arm gate, VERSION probe, arm-variant scanner, and the
    // 24-candidate ArmScan were removed in the RTCore64 swap — RTCore64
    // has no arm sequence; the first read IOCTL after CreateFile
    // dispatches straight through the driver's DEVICE_CONTROL handler.)

    // (ReadScan / ValidateReadCandidate deleted — they enumerated wire
    // shapes for the previous driver's read struct. RTCore64's request
    // struct is fixed at RTC_REQ_SIZE bytes; no shape scan needed.)

    // (ReadIoctlSweep and TryArmWithRetries deleted — the swept ranges
    // and the arm-retry sequence were specific to NTIOLib. RTCore64's
    // read primitive is a single documented IOCTL with a fixed struct.)

    // Physical read via RTCore64 IOCTL 0x80002048. Each dispatch reads a
    // single granularity-sized chunk (1, 2, 4, or 8 bytes) from a mapped
    // physical page via MmMapIoSpace. The driver's on-wire request is a
    // fixed 0x30-byte METHOD_BUFFERED struct where input and output share
    // the same system buffer; the data slot starts at RTC_REQ_DATA_OFFSET.
    // Larger reads loop this call across `count` units with an advancing
    // physical address.
    //
    // Failure discipline:
    //   - Transient (err=1450)   → retry once
    //   - Pipe-death (err=5/6)   → count toward the circuit breaker
    //   - Per-PA rejections      → reset streak; the driver is alive,
    //                              this specific address isn't mappable
    bool RtcReadOnce(uint64_t physAddr, uint32_t unit_size, uint32_t count,
        ...
        SivFail  cls = SivFail::Ok;
        DWORD    lastReturned = 0;
        DWORD    lastErr      = 0;

        for (uint32_t i = 0; i < count; i++) {
            uint8_t req[RTC_REQ_SIZE] = {};
            *(uint64_t*)(req + 0x08) = physAddr + (uint64_t)i * unit_size;
            *(uint32_t*)(req + 0x14) = unit_size;
            // bytes_processed at +0x10, data at +0x18, length at +0x28: driver
            // fills those; we leave zero on the way in.

            uint8_t rsp[RTC_REQ_SIZE] = {};
            DWORD   returned = 0;
            SetLastError(0);
            BOOL ok = DeviceIoControl(hUse, IOCTL_RTC_READ_PHYS,
                                      req, RTC_REQ_SIZE,
                                      rsp, RTC_REQ_SIZE,
                                      &returned, nullptr);
            lastReturned = returned;

            // Transient retry (single shot) — a kernel low-resources burst
            // shouldn't propagate up to the page cache as a permanent error.
            if (!ok && GetLastError() == 1450) {
                Sleep(0);
                SetLastError(0);
                ok = DeviceIoControl(hUse, IOCTL_RTC_READ_PHYS,
                                     req, RTC_REQ_SIZE,
                                     rsp, RTC_REQ_SIZE,
                                     &returned, nullptr);
                lastReturned = returned;
            ...
            if (returned < RTC_REQ_SIZE) {
                lastErr = GetLastError();
                cls = SivFail::PartialReturn;
                goto after_loop;
            }
            memcpy(dst + (size_t)i * unit_size, rsp + RTC_REQ_DATA_OFFSET, unit_size);
        }
        cls = SivFail::Ok;

after_loop:
        // HandleStale (err=6) means the driver was reloaded out from under
        // us — close the handle so the idle-mode reopen builds a fresh one.
        ...
        NoteSivFail(cls, physAddr, (DWORD)bytes, lastReturned, lastErr);
        if (IsPipeDeath(cls)) {
            ...
            if (streak >= kIoFailStreakBreaker) {
                if (!m_ioBroken.exchange(true, std::memory_order_acq_rel)) {
                    printf(skCrypt("[SysMonitor] RTCore64 pipe dead (%d consecutive %s failures) — fast-failing further reads. Reset via WinDrvReader::Get().ResetIoBreaker() after re-loading.\n"),
    ...
    // Arbitrary-size physical read via RtcReadOnce.
    // RTCore64 dispatches one granularity-sized read per IOCTL (max 8B);
    // this wrapper picks the largest granularity that both PA and size
    // align to, so a 4K page fill is 512 dispatches at unit=8 rather
    // than 4096 at unit=1.
    bool SivReadPhys(uint64_t physAddr, void* out, size_t size) {
        ...
        // PA sanity: top 16 bits ≤ 0xFEDC leaves ~255 TB, plenty of
        // headroom for anything short of a kernel-VA-shaped garbage value.
        if (((physAddr + size - 1) >> 48) > 0xFEDCULL) {
            NoteSivFail(SivFail::BadArgs, physAddr, (DWORD)size, 0, 0);
            return false;
        }

        // Pick the largest unit_size that both PA and size align to.
        uint32_t unit;
        uint32_t count;
        if ((physAddr & 7ULL) == 0 && (size & 7ULL) == 0) {
            unit = 8; count = (uint32_t)(size / 8);
        } else if ((physAddr & 3ULL) == 0 && (size & 3ULL) == 0) {
        ...
        return RtcReadOnce(physAddr, unit, count, out, size);
    }

    // Fills a 4 KB physical page. Under RTCore64 this issues 512 8-byte
    // dispatches (or 1024 4-byte on unaligned PAs). Slower than NTIOLib's
    // bulk primitive but still fast enough for the cache thread's cold
    // fills; steady-state ESP polling hits the page cache in memcpy.
    bool SivReadPage(uint64_t pagePA, uint8_t out[4096]) {
        pagePA &= ~0xFFFULL;
        if (!out || reinterpret_cast<uintptr_t>(out) < 0x10000ULL) return false;

        // Same PA-sanity gate (top 16 bits ≤ 0xFEDC).
        ...
        // Serialise so one thread's IOCTL burst isn't interleaved with another's.
        EnterCriticalSection(&m_ioctlLock);
        // Page is 8-byte aligned; use qword granularity for 512 dispatches.
        bool ok = RtcReadOnce(pagePA, 8, 512, out, 4096);
        if (!ok) {
            static bool s_logged = false;
            if (!s_logged) {
                s_logged = true;
                printf(skCrypt("[SysMonitor] RTCore64 page read failed: err=%lu (PA=0x%llX)\n"),
    ...
    // headroom. Cold-page fills under RTCore64 cost 512 IOCTLs per page
    // (8-byte granularity), so hitting cache instead of re-filling is
    // even more valuable than under the NTIOLib bulk primitive. 256 slots
    // captures the CS2 entity working set on high-player-count rounds
    // (up to 10 controllers × ~5 hot pages each + a dozen weapon/globals
    // pages) with room for VM/EPROCESS page reuse. Steady-state cost:
    // +512 KB heap.
